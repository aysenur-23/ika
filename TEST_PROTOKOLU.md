...
| **3** | **Negatif engel / çukur** (zemin altına düşme riski) | **Derinlik tabanlı:** `dropoff_platform` (arka), **`pit_platform_forward`** (ön). **Görsel:** `pothole_visual_{1,2}`, `trench_visual_1`, `hazard_cone_{1,2}` (RGB sınıflandırıcı için, §5.1) | Derinlik: `ika_terrain` RANSAC → `DROPOFF_DANGER`. Görsel: gelecekteki RGB classifier (Sınıf 7) | `/terrain_state.class=DROPOFF_DANGER` → fusion `STOP` | Robot platform kenarına yaklaşınca durur (≥ 15 cm derinlik farkı). Görsel pothole/trench RGB classifier eklenince RISKY sınıfında |
